...
beta = sparse(train_label(train(:,1)),train(:,2),...
    train(:,3)./n_word_class(train_label(train(:,1))));
beta_log = log(beta);
s_test_train = s_test(:,1:n_train_unique_word);
...
MLE_class = s_test_train*beta_log' + ones(n_test_docs,1)*py_log';
for j = 1:n_test_docs
    LE_log = MLE_class(j,:);
    if max(LE_log) == -Inf
